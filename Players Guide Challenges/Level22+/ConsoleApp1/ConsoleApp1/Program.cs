...
int range = random.Next(1, 101);
Console.Clear();

Console.WriteLine($"random range is {range}");
...
while (cityHealth > 0 && manticoreHealth > 0)
{
    ...
    if (targetRange == range) { manticoreHealth -= damage; range = random.Next(1, 101); Console.WriteLine($"Random Range has changed to {range} "); }
